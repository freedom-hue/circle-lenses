...
# =========================================================
# 💡 2. 获取 exe 同级目录真实路径（保存 Customer Profile 档案专用）
# =========================================================
def get_exe_dir():
    if getattr(sys, 'frozen', False):
...
class EyeFittingEngine:
    ...
    def _render_lens(self, frame, center, iris_radius, eyelid_pts, config, texture_cache):
        cx, cy = center
        radius = int(iris_radius * config.get("scale", 1.15))
        if radius <= 0: return
        ...
        if preset_type == "image":
            ...
            cv2.circle(lens_img, (lens_center, lens_center), radius, (*preset["outer"], 255), -1)
            cv2.circle(lens_img, (lens_center, lens_center), int(radius * 0.78), (*preset["inner"], 255), -1)
...
class AdvancedLensStudio(QMainWindow):
    ...
    def _build_fitting_tab(self):
        ...
        self.slider_scale.setValue(115)
    ...
    def upload_local_image(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "选择自拍照", "", "Images (*.png *.jpg *.jpeg)")
        if file_path:
    ...
    def capture_and_save_workflow(self):
        ...
        # 🔧 修改处：使用 get_exe_dir 保存保存档案到真实位置
    ...
    def run_ai_recommendation(self):
        msg = "✨ AI 智能诊断：推荐【03. 清晨雾灰】与【微醺橄榄绿】，极具水光感！"
